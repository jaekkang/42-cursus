...
bool isNumericString(const std::string &str) {
  ...
  for (; i < str.length(); i++) {
    if (str[i] == '.') {
      check += 1;
      if (check > 1 || i + 1 == str.length()) return false;
    }
...
int moreComma(std::string input) {
  ...
  while (input[i]) {
    if (input[i] == '.') check += 1;
    if (check > 1) throw ScalarConverter::InvalidInput();
    i++;
  }
  return 0;
}
...
int checkInput(std::string input) {
  ...
  for (size_t i = 0; i < input.length(); i++) {
    if (!isdigit(input[i]) && input[i] != '.' && input[i] != 'f') return 0;
    if (isdigit(input[i])) d += 1;
    if (isalpha(input[i]) && input[i] != 'f') a += 1;
  }
  if (a > 0 && d > 0) return (0);
  return 1;
}
...
int check_float(std::string input) {
  ...
  pos = input.find('.');
  if (input[input.length() - 1] != 'f' || pos == std::string::npos) return 0;
  for (size_t i = pos; pos < input.length(); i++) {
    if (isalpha(input[i]) && input[i] != 'f') return 0;
    if (input[i] == 'f') {
      f += 1;
      if (f > 1) return 0;
    }
...
void ScalarConverter::convert(std::string input) const {
  ...
  try {
    ...
    if (c == '.') {
      if (input.length() == 1) throw ScalarConverter::InvalidInput();
    ...
    } else if (moreComma(input) ||
               (input.find('.') == std::string::npos && !checkInput(input))) {
      throw ScalarConverter::InvalidInput();
    } else if (input != "nan" && input != "nanf" && isNumericString(input)) {
      tmp = std::stod(input);
      if (check_char_invalid(tmp)) {
        std::cout << "char: '" << static_cast<char>(tmp) << "'" << std::endl;
      } else if (static_cast<int>(tmp) >= 0 && static_cast<int>(tmp) <= 127) {
        std::cout << "char: Non displayable" << std::endl;
      } else {
        std::cout << "char: impossible" << std::endl;
      }
      if (check_int_invalid(tmp)) {
        std::cout << "int: " << static_cast<int>(myRound(input)) << std::endl;
      } else {
        std::cout << "int: impossible" << std::endl;
      }
    } else if ((pos = input.find('.')) != std::string::npos) {
      if (!checkInput(input)) throw ScalarConverter::InvalidInput();
    ...
    if ((pos = input.find('.')) == std::string::npos && input != "nan" &&
      ...
      if (input[input.length() - 1] == 'f') {
        ...
        std::cout << "float: " << std::fixed << std::setprecision(fpos.length())
                  << tmp << "f" << std::endl;
        std::cout << "double: " << tmp << std::endl;
      }
    }
